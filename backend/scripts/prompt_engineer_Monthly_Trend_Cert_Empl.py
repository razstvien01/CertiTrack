...
def generate_prompt(result_list):
    ...
    Provide the monthly trend of certified employees for Fiscal Year 2024. List them in January to December order.
    I need the output as a JSON object in this structure {{"Fiscal_Year": "FY24", "Buwan": "Month Name", "CertifiedEmployees": "Count of Certified Employees"}} and no other explanations.
    """
...
if __name__ == "__main__":
    ...
    query = """
    SELECT Month, COUNT(*) AS number_Passed
    FROM database_table
    WHERE Fiscal_Year = 'FY24' AND CURRENT_PROGRESS = 'Passed'
    GROUP BY Month ;
    """
    ...
    table_name = "Monthly_Trend_Cert_Empl"
    dml_statements = generate_dml_script(table_name, json_data)

    # Write DML statements to a file
    dml_file_name = 'generated_dml_for_Monthly_Trend_Cert_Empl.sql'
    ...
    ddl_file_path = 'backend/SQL/DDL/create_Monthly_Trend_Cert_Empl.sql'
