...
def generate_prompt(result_list):
    ...
    Can you give me the summary presenting the overall completion rates of certifications for the past year (FY23). Show any significant changes or trends compared to previous Fiscal Years (FY22, FY21, FY20).
    I only need the JSON list in this structure {{"FiscalYear": "Fiscal Year", "Completion_Rate": "Completion Rate", "FY22" : "Previous Years Completion rates", "FY21" : "Previous Years Completion rates", "FY20" : "Previous Years Completion rates"}} and no other explanations.
    """
...
if __name__ == "__main__":
    ...
    query = """
    SELECT Fiscal_Year,
           ROUND(SUM(CASE WHEN CURRENT_PROGRESS = 'Passed' THEN 1 ELSE 0 END) * 1.0 / COUNT(*) * 100, 2) AS completion_rate
    FROM database_table
    WHERE Fiscal_Year IN ('FY22', 'FY21', 'FY20', 'FY23')
    ...
    json_data = json.loads(response) #Response should have double quotes in them. So edit in the structure format in the promp

    # Generate DML statement for the Selected Table
    # table_name = os.path.splitext(os.path.basename(empty_table_ddl_file))[0]
    table_name = "OverallCompletion"
    dml_statements = generate_dml_script(table_name, json_data)

    # Write DML statements to a file
    dml_file_name = 'generated_dml_for_OverallCompletion.sql'
    ...
    ddl_file_path = 'backend/SQL/DDL/create_OverallCompletion.sql'
